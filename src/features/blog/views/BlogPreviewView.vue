<script setup lang="ts">
import { computed } from 'vue'
import { usePageSeo } from '@/features/home/composables/usePageSeo'
import BlogPreviewPosts from '../components/BlogPreviewPosts.vue'
import { useBlogPreview } from '../composables/useBlogPreview'

const { data, loading, error, refresh } = useBlogPreview()

usePageSeo(() =>
  data.value?.seoTitle
    ? {
        metaTitle: data.value.seoTitle,
        metaDescription: data.value.description || undefined,
      }
    : undefined,
)

const heading = computed(() => data.value?.heading ?? '')
const description = computed(() => data.value?.description ?? '')
const posts = computed(() => data.value?.posts ?? [])
</script>

<template>
  <div class="min-h-screen">
    <div class="pointer-events-none fixed inset-0 z-0 grid-bg" />

    <main class="relative z-10 min-h-screen">
      <div
        v-if="loading"
        class="flex min-h-screen items-center justify-center text-sm uppercase tracking-widest text-on-surface-variant"
      >
        Cargando…
      </div>

      <div
        v-else-if="error"
        class="flex min-h-screen flex-col items-center justify-center gap-4 px-6 text-center"
      >
        <p class="text-on-surface">{{ error.message }}</p>
        <button
          type="button"
          class="rounded border border-white/30 px-4 py-2 text-sm uppercase tracking-widest text-on-surface hover:bg-white/10"
          @click="refresh"
        >
          Reintentar
        </button>
      </div>

      <BlogPreviewPosts
        v-else-if="data"
        :heading="heading"
        :description="description"
        :posts="posts"
      />
    </main>
  </div>
</template>
