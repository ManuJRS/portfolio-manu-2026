...
import BlogArticleBlockRenderer from '../components/BlogArticleBlockRenderer.vue'
import { useBlogArticle } from '../composables/useBlogArticle'

const route = useRoute()

const articleSlug = computed(() => String(route.params.slug ?? ''))

const { data, loading, error, refresh } = useBlogArticle(articleSlug)

usePageSeo(() =>
  data.value?.title
    ? {
        metaTitle: data.value.title,
      }
    : undefined,
)
</script>
...
<template>
  <div class="min-h-screen">
    ...
    <main class="relative z-10 min-h-screen">
      ...
      <article v-else-if="data?.sections.length" class="mx-auto max-w-5xl px-6 py-20 md:px-8 md:py-28">
      ...
      >
        No hay contenido para este artículo.
      </p>
    </main>
  </div>
</template>
